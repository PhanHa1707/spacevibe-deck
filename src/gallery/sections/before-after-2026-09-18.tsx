  ...
  checkout({
    ...
    panes: [
      pane({
        paneId: 4,
        agent: "codex",
        label: "Adding the contract test for pty_kill_foreground",
        state: "working",
        tabIndex: 3,
        focused: true,
      }),
...
function codexPair() {
  return (
    <div class="gx-ba">
      <Column
        title="dark · unchanged"
        note="reference: the white mark reads on deck-dark, so the strip and the open row both identify Codex."
      >
      ...
      <Column
        title="light · before (ships)"
        note="the same rail on deck-light: `agent-codex.svg` is `fill=#fff` in a plain img element, so the segment, the ×2 and the open row show a blank disc."
      >
      ...
      <Column
        title="light · after (candidate)"
        note="the mark painted in currentColor: the same SVG as a mask over the row's own text colour, so it follows the theme like every other chrome tone (DL-2.2) and uses no `filter` (DL-1.3). Production would inline the SVG through AgentGlyph with fill=currentColor, keeping the coloured marks as img elements."
        candidate="codex-ink"
      >
        <ThemeScope themeId={DECK_LIGHT_ID}>
          <CardRail groups={CODEX_GROUPS} openKeys={CODEX_OPEN} />
...
function namePair() {
  return (
    <div class="gx-ba">
      <Column
        title="before (ships)"
        note="`.asr-card__head` is `14px minmax(0,1fr) auto 13px`: the badge's auto column takes its full width first, so the checkout's own name is what truncates (`redesi…`) while `redesign/phase-1-2` stays whole — the opposite of what the badge comment in 04c says it protects."
      >
        <CardRail groups={NAME_GROUPS} openKeys={NO_OPEN} />
      </Column>
      <Column
        title="after (candidate)"
        note="the name gets a floor and the badge yields: `minmax(7ch,1fr)` for the name, `minmax(0,auto)` and a 96px cap for the badge, which already ellipsizes its own text. Change lives in `src/styles/04c-rail-worktree-card.css` (head grid + badge max-width); DL-27.25 is unchanged."
  ...
  checkout({
    ...
    panes: [
      pane({
        paneId: 21,
        agent: "claude",
        label: "Reading the rail model to see where the turn line is built",
        state: "working",
        tabIndex: 0,
        focused: true,
      }),
      pane({
        paneId: 22,
        agent: "codex",
        label: "Should I overwrite the existing migration or create a new one?",
        state: "asked",
        tabIndex: 1,
      }),
    ],
  ...
  checkout({
    ...
    panes: [
      ...
      pane({
        paneId: 24,
        agent: "opencode",
        label: "Done — both hosts read the same journal now",
        state: "done",
        tabIndex: 3,
      }),
    ],
...
function restPair() {
  return (
    <div class="gx-ba">
      <Column
        title="before (ships)"
        note="every card starts closed (`openCardKeys` is an empty Set per window, never persisted) and a closed card is glyph segments, ×N, an age and a +. Five panes, one asked and one failed, and not one word of what any agent said; the two that need you are two pinhead badges."
      >
        <CardRail groups={REST_GROUPS} openKeys={NO_OPEN} />
      </Column>
      <Column
        title="after (candidate)"
        note="cards with a live pane open by default, so each row prints the pane's newest sentence (what production already prints for an unnamed pane when the card is open), and one `Needs me N` line above the cluster. Change: `src/ui/agent-rail.tsx` seeds `openCardKeys` from live checkouts (or persists it); the count line is new markup. Fork: DL-27.25's closed-by-default clause. The strip-chip half of this finding (chips naming the place instead of the sentence) is not drawn here."
        candidate="rail-open"
      >
        <NeedsMeLine count={2} />
...
const BOARD_ROWS = [
  {
    ...
    tail: "Should I overwrite the existing migration or create a new one?",
    changed: "2m",
  },
  {
    paneId: 32,
    ...
    tail: "Adding the contract test for pty_kill_foreground",
    changed: "1m",
  },
  {
    paneId: 33,
    ...
    tail: "Build failed: tsc exit 2",
    changed: "5m",
  },
];

function boardCards(identity: (row: (typeof BOARD_ROWS)[number]) => BoardIdentity): BoardCard[] {
  return BOARD_ROWS.map((row, index) => boardCard({ ...row, rank: index + 1, ...identity(row) }));
}
...
function boardPair() {
  return (
    <div class="gx-ba">
      <Column
        title="before (ships)"
        note="three cards, one checkout: the identity line is glyph + project (DL-34.2), so all three read `spacevibe-deck` and only a 17px logo separates Claude from Codex — and nothing separates Claude from Claude 2 but the rank."
      >
